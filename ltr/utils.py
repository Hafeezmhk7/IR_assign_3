...
from .model import LTRModel, CLTRModel
...
def create_results(data, net, train_fn, prediction_fn, results_file, *train_params):
    ...
    test_q = {}
    metric_keys = (
        train_params[0].metrics
        if train_params
        else {"ndcg", "precision@05", "recall@05"}
    )

    for m in metric_keys:
        test_q[m] = test_qq.get(m, 0)  # Ensure key exists in test_qq
        print(f'\t"{m}": {test_metrics.get(m, 0)}')  # Avoid KeyError
